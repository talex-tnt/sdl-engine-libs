...
parser.add_option("--chars",dest="characters",help="List of characters to include. Defaults to a reasonable list of mixed case alphanumerics and punctuation.",default="ABCDEFGHIJKLMNOPQRSTUVWXYZabcdefghijklmnopqrstuvwxyz 0123456789,.:!?@#$^&*()[]\"\'-/<>{}+;=~\\_")
parser.add_option("-x","--extra",dest="extra_characters",help="List of extra characters to include." ,default="")
...
parser.add_option("--sprites",dest="spritesfile",help="File that maps Glyphs to Sprites Names",default=None)
...
spritenames = None
if options.spritesfile:
	with open(options.spritesfile + ".json") as jsonfile:
		spritenames = json.load(jsonfile)
		for k, v in spritenames.items():
			print(k + '-' + v)
...
def render(do_save):
	fontNameOnly = options.font or options.spritesfile

	fontname = fontNameOnly +".ttf"
	...
	chars =  bytes(options.characters,"UTF8", "strict") + bytes(options.extra_characters, "UTF8","strict")
	if options.unicode:
	  chars = chars + bytes("%éüùú£¢¡¿àáñöóòïíèáéíóúüñ","UTF8")
	image_filename = fontNameOnly+"-"+str(fontsize)+".png"
	output = "font"
	if options.plist:
		output = "plist"
	...
	if spritenames:
		output = "sprites"
		chars = bytes(''.join(spritenames.keys()), "UTF8", "strict")

	text_filename = fontNameOnly+"-"+str(fontsize)+"."+output
	...
	if do_save:
		...
		if output == "plist":
			...
			fout.write("<dict>\n")
	for c in chars:
		print('printing: ' + chr(c))
		try:
			charsurf = font.render(chr(c),antialias,color)
			...
			if do_save:
				if output == "plist":
					...
					fout.write(chr(c) + "\t" + str(x) + "\t" + str(surfsize[1]-y) + "\t" + str(charsize[0]) + "\t" + str(charsize[1]) + "\n")
				elif output == "sprites":
					fout.write(spritenames[chr(c)] + "\t" + str(x) + "\t" + str(surfsize[1]-y) + "\t" + str(charsize[0]) + "\t" + str(charsize[1]) + "\n")
